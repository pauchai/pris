...
<?php
if ($model->program) {
    foreach ($model->program->getProgramVisits()->distinctDates() as $dateValue) {
        $gridColumns[] = [
            'header' => $dateValue,
                    ...
                    return Html::tag('div',
                        ...
                        [
                            'data-model' => \yii\helpers\Json::encode([
                                'program_prisoner_id' => $model->promaryKey,
                                'date_visit' => $dateValue
...
<?php
$dataProvider = new \yii\data\ActiveDataProvider(['query'=>\vova07\plans\models\ProgramPrisoner::find()->where(['__ownableitem_id'=>$model->primaryKey])]);
echo \yii\grid\GridView::widget(['id' => 'participants','dataProvider' => $dataProvider,
    'columns' => $gridColumns,
])?>
